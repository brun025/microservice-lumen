...
parser(){
  ...
  if [ -z $ENV ] ; then
    read -p 'Env, digit one of dev,pro: ' ENV
  fi
...
download_wp(){
  ...
  curl -LkSs https://github.com/marco-gallegos/lumen-rest-api-template/archive/master.zip -o lumen.zip
  
  unzip lumen.zip

  rm lumen.zip

  mv lumen-rest-api-template-master ${SOURCEPATH}
